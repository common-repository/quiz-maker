...
class AYS_Quiz_Admin{
    ...
    public function setup_constants() {
        ...
        if(!defined('AYS_FILE')){
                define( 'AYS_FILE', AYS_DIR . 'ays_quiz_admin.php' );
        }
    }
    function yntrel(){
        global $wpdb;
    ...
    public function ays_quiz_menu_generate(){
        ...
        add_menu_page('Quiz Maker', 'Quiz Maker', 'manage_options', 'ays_quiz_main',array($this,'ays_quiz_main'));
    ...
    public  function ays_quiz_main(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Main.php' );
        AYS_Quiz_Main::AYS_Main();
    }
    public  function ays_quiz_quiz_categories(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Quiz_Categories.php' );
    ...
    public  function ays_quiz_quizes(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Quizes.php' );
        if (isset($_GET["task"]) && $_GET["task"] == "add_or_edit"){
    ...
    public  function ays_quiz_question_categories(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Question_Categories.php' );
        if (isset($_GET["task"]) && $_GET["task"] == "add_or_edit"){
    ...
    public  function ays_quiz_questions(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Questions.php' );
        if (isset($_GET["task"]) && $_GET["task"] == "add_or_edit"){
    ...
    public  function ays_quiz_results(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Results.php' );
    ...
    public  function ays_quiz_themes(){
        include_once( AYS_DIR . '/classes/AYS_Quiz_Themes.php' );
        if (isset($_GET["task"]) && $_GET["task"] == "add"){
                AYS_Quiz_Themes::add();
        }
        else if(isset($_GET["task"]) && $_GET["task"] == "edit" ){
    ...
    public function ays_quiz_admin_scripts_styles(){
        ...
        wp_register_style('ays_admin_style',AYS_URL.'/includes/css/jquery-ui.css');
        wp_enqueue_style( 'ays_admin_style');
        wp_register_style('ays_admin_style1',AYS_URL.'/includes/css/jquery.minicolors.css');
        wp_enqueue_style( 'ays_admin_style1');
        wp_register_style('ays_admin_style2',AYS_URL.'/includes/css/style.css');
        wp_enqueue_style( 'ays_admin_style2');
        wp_register_style('ays_admin_style3',AYS_URL.'/includes/css/toolbar.css');
        wp_enqueue_style( 'ays_admin_style3');
        wp_enqueue_script( 'jquery-ui-sortable');
        wp_register_script('ays_admin_script',AYS_URL.'/includes/js/aysquiz_jquery.js');
        wp_enqueue_script( 'ays_admin_script' );
        wp_register_script('ays_admin_script1',AYS_URL.'/includes/js/deafult_layout.js');
        wp_enqueue_script( 'ays_admin_script1' );
        wp_register_script('ays_admin_script2',AYS_URL.'/includes/js/jquery.minicolors.js');
        wp_enqueue_script( 'ays_admin_script2' );
        wp_register_script('ays_admin_script3',AYS_URL.'/includes/js/jquery-ui.js');
