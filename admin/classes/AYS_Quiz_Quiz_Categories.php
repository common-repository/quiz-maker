<?php
class AYS_Quiz_Quiz_Categories{
    ...
    public static function find( $args = '' ) {
        ...
        $rows = $wpdb->get_results("SELECT * FROM ".$wpdb->prefix."aysquiz_quizcategories ".$where.$oderby , OBJECT);

        return $rows;
    }    
    public function ays_quiz_cat_display_list(){
        include_once AYS_DIR.'/classes/AYS_Quiz_Quiz_Cat_List_Tables.php';
        $list_table = new AYS_Quiz_Main_List_Table();
        $list_table->prepare_items();
        ?>
        <div class="wrap">
                <h2>
                        <?php
                                echo esc_html( __( 'AYS Quiz Categories', 'ays' ) );
                                echo ' <a href="admin.php?page=ays_quiz_quiz_categories&task=add_or_edit" class="add-new-h2">' . esc_html( __( 'Add Category', 'ays' ) ) . '</a>';
                                if ( ! empty( $_REQUEST['s'] ) ) {
    ...
    function ays_qu_cat_updated_message() {
            ...
            if ( 'created' == $_REQUEST['message'] )
                    $updated_message = esc_html( __( 'Quiz category created.', 'ays_quiz_quiz_categories' ) );
            elseif ( 'saved' == $_REQUEST['message'] )
                    $updated_message = esc_html( __( 'Quiz category saved.', 'ays_quiz_quiz_categories' ) );
            elseif ( 'deleted' == $_REQUEST['message'] )
                    $updated_message = esc_html( __( 'Quiz category deleted.', 'ays_quiz_quiz_categories' ) );
    ...
    public function add_or_edit(){
        include_once AYS_DIR.'/helpers/helper.php';
        global $wpdb;
        ...
        <div class="wrap">
            <?php
                if($ays_quiz_cat_id==0){
                    echo '<h1>Add quiz new category</h1>';
                }
                else{
                    echo '<h1>Edit quiz category</h1>';
                }
            ?>
        ...
        <?php
            if(isset($_POST["ays_quiz_cat_edit_options"])){
                ...
                AYS_Quiz_Helper::ays_redirect("?page=ays_quiz_quiz_categories");
            ...
            if(isset($_POST["ays_quiz_cat_apply_options"])){
                ...
                if($ays_quiz_cat_id==0){
                    ...
                    AYS_Quiz_Helper::ays_redirect("admin.php?page=ays_quiz_quiz_categories&task=add_or_edit&q_id=".$ays_last_id."");
                }
                else{
                    $wpdb->update(
                        $ayq_quiz_category_table,
                    ...
                    AYS_Quiz_Helper::ays_redirect("admin.php?page=ays_quiz_quiz_categories&task=add_or_edit&q_id=".$ays_quiz_cat_id);
                }
            }
    ...
    public function delete() {
        ...
        $query = "DELETE FROM ".$wpdb->prefix."aysquiz_quizcategories WHERE id = ".$this->id;
        $wpdb->query($query);
        $this->id = 0;
    }
    public static function get_instance( $qu_cat ) {
            global $wpdb;
            $row = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix."aysquiz_quizcategories WHERE id=".(int)$qu_cat, OBJECT);
